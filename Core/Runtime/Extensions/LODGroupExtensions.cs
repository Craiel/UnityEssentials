namespace Craiel.UnityEssentials.Runtime.Extensions
{
    ...
    public static class LODGroupExtensions
    {
        ...
        {
            LOD[] lods = lodGroup.GetLODs();
            float relativeHeight = lodGroup.GetRelativeHeight(camera ? camera : Camera.current);
            int lodIndex = GetCurrentLOD(lods, lodGroup.GetMaxLOD(), relativeHeight);
            return lodIndex;
        }
        ...
        static int GetCurrentLOD(LOD[] lods, int maxLOD, float relativeHeight)
        {
            int lodIndex = maxLOD;
            for (var i = 0; i < lods.Length; i++)
            {
        ...
        static float GetWorldSpaceScale(Transform transform)
        {
